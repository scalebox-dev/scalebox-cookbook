...
def test_bedrock_connection():
    ...
    print(f"AWS_BEDROCK_TOKEN: {'***' if os.getenv('AWS_BEDROCK_TOKEN') else 'Not set'}")
    print()
    
    # 如果有 Bearer Token，设置为 Session Token
    bearer_token = os.getenv('AWS_BEDROCK_TOKEN')
    if bearer_token:
        print("检测到 AWS_BEDROCK_TOKEN，将其设置为 AWS_SESSION_TOKEN")
    ...
    try:
        ...
        if "AccessDeniedException" in str(e):
            print("• 认证失败，可能的原因：")
            print("  1. AWS_BEDROCK_TOKEN 不正确或已过期")
